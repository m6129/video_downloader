...
class UrlError(RutubeError):
    ...


class DownloadError(RutubeError):
    ...


class SegmentUrlsError(RutubeError):
    ...


class SegmentDataError(RutubeError):
    ...


class VideosError(RutubeError):
    ...


class VideoIdError(RutubeError):
    ...
...
class Rutube:
    ...
    def _get_video_id(self):
        ...
        if result:
            return result[0]
        raise VideoIdError('Cannot get the video ID from URL')
    ...
    def _check_url(self):
        if requests.get(self._video_url).status_code != 200:
            raise UrlError(f'{self._video_url} is unavailable')
        return True
...
class YappyPlaylist(BasePlaylist):
    ...
    def _get_videos(self) -> list:
        ...
        if r.status_code != 200:
            raise VideosError(f'Error code: {r and r.status_code}')

        results: list = r.json().get('results')
        if not results:
            raise VideosError('No results found')
...
class YappyVideo:
    ...
    def download(self):
        ...
        if r.status_code != 200:
            raise DownloadError(f'Error code: {r and r.status_code}')
...
class RutubeVideo:
    ...
    def _get_segment_urls(self):
        ...
        if r.status_code != 200:
            ...
            if r.status_code != 200:
                raise SegmentUrlsError(f'Cannot get segments. Status code: {r.status_code}')
    ...
    def _get_segment_data(uri):
        ...
        while retry:
            ...
            retry -= 1
            time.sleep(TIMEOUT)
        raise SegmentDataError(f'Error code: {r and r.status_code}')

    @staticmethod
    def _build_file_path(filename: Text, path: Text = None) -> str:
        if not path:
            return filename
    ...
    def download(self, path: Optional[Text] = None, stream: Optional[BinaryIO] = None, filename: Optional[Text] = None):
        filename = filename or f'{self.title}.mp4'
        if stream:
            self._write(stream)
        else:
            with open(self._build_file_path(path=path, filename=filename), 'wb') as file:
                self._write(file)
